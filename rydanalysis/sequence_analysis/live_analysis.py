...
class LiveAnalysis:
    def __init__(
        ...
        if raw_data is not None:
            ...
            self.ion_description, self.fit_ds, self.summary = self.analyze_data(
                raw_data
            )
        else:
    ...
    def analyze_data(self, raw_data):
        ion_description, ion_summary = self.ion_analysis.full_analysis(
            raw_data.scope_traces
        )
    ...
    def update(self, raw_data):
        ...
        self.ion_description = pd.concat(
            [self.ion_description, new_description], axis=0
        )
