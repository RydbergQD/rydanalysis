...
class Fit2d2Gaussian(Fit2d):
    ...
    @staticmethod
    def _function(args, amp1, cen_x1, cen_y1, sig_x1, sig_y1, theta1, amp2, cen_x2, cen_y2, sig_x2, sig_y2, theta2, offset):
        x, y = args
        # cen_x1 = float(cen_x1)
        # cen_y1 = float(cen_y1)
        # cen_x2 = float(cen_x2)
        ...
        g = offset + amp1*np.exp( - (a1*((x-cen_x1)**2) + 2*b1*(x-cen_x1)*(y-cen_y1) + c1*((y-cen_y1)**2))) + amp2*np.exp( - (a2*((x-cen_x2)**2) + 2*b2*(x-cen_x2)*(y-cen_y2) + c2*((y-cen_y2)**2)))
